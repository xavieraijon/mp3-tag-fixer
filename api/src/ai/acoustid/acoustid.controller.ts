  ...
  async identifyTrack(
    ...
    if (!file) {
      throw new BadRequestException(
        'File not found or expired. Please upload the file first.',
      );
    }
    ...
    if (!this.acoustidService.isAvailable()) {
      return {
        error:
          'AcoustID service not available. Check ACOUSTID_API_KEY configuration.',
      };
    }
    ...
    if (!result) {
      return {
        error:
          'Could not identify track. Audio fingerprint not found in database.',
      };
    }
