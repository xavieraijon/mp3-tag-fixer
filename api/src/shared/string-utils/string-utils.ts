/**
 * Centralized string utilities for MP3 tag processing.
 * This is the single source of truth for string normalization across the application.
 *
 * Used by: DiscogsModule, CorrectionModule, FilesModule
 */
export class StringUtils {
  ...
  /**
   * Strict normalization for knowledge base matching.
   * NFKD normalized, lowercase, only alphanumeric.
   */
  static normalizeStrict(text: string): string {
    return text
      .normalize('NFKD')
  ...
  /**
   * Generates a normalized key for knowledge base lookups.
   * Format: "normalizedartist|normalizedtitle"
   */
  static generateNormalizedKey(artist: string, title: string): string | null {
    if (!artist || !title) return null;
    return `${this.normalizeStrict(artist)}|${this.normalizeStrict(title)}`;
  }
}
