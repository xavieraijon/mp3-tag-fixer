...
const BUTTON_STYLES: Record<ButtonVariant, Record<ButtonPalette, string>> = {
  solid: {
      primary: 'bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 border border-transparent',
      neutral: 'bg-slate-800 hover:bg-slate-900 text-white shadow-lg shadow-slate-500/20 border border-transparent',
      blue:    'bg-blue-500 hover:bg-blue-600 text-white shadow-lg shadow-blue-500/20 border border-transparent',
      purple:  'bg-purple-500 hover:bg-purple-600 text-white shadow-lg shadow-purple-500/20 border border-transparent',
      emerald: 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/20 border border-transparent',
  ...
  glass: {
      primary: 'backdrop-blur-2xl backdrop-saturate-150 bg-white/40 hover:bg-indigo-50/60 text-slate-700 hover:text-indigo-700 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.5)] hover:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.7)] border border-white/50 hover:border-indigo-300/50 transition-all duration-300',
      neutral: 'backdrop-blur-2xl backdrop-saturate-150 bg-white/40 hover:bg-white/60 text-slate-700 hover:text-slate-900 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.5)] hover:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.7)] border border-white/50 hover:border-slate-400/50 transition-all duration-300',
      ...
      purple:  'backdrop-blur-2xl backdrop-saturate-150 bg-purple-500/10 hover:bg-purple-500/30 text-purple-700 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.5)] hover:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.7)] border border-white/50 hover:border-purple-400/50 transition-all duration-300',
      emerald: 'backdrop-blur-2xl backdrop-saturate-150 bg-emerald-500/10 hover:bg-emerald-500/30 text-emerald-700 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.5)] hover:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.7)] border border-white/50 hover:border-emerald-400/50 transition-all duration-300',
  ...
  outline: {
      primary: 'border-2 border-slate-200 hover:border-indigo-500 text-slate-600 hover:text-indigo-600',
      ...
      purple:  'border-2 border-slate-200 hover:border-purple-500 text-slate-600 hover:text-purple-600',
      emerald: 'border-2 border-slate-200 hover:border-emerald-500 text-slate-600 hover:text-emerald-600',
...
@Component({
  ...
  template: `
    ...
    >
      @if (icon()) {
        <lucide-icon [name]="icon()!" [size]="iconSize()" [class.mr-2]="!isIconOnly()" [class.mr-0]="isIconOnly()"></lucide-icon>
      }
...
export class ButtonComponent {
  ...
  computedClasses = computed(() => {
    const base = 'inline-flex items-center justify-center font-bold transition-all focus:outline-none';
    ...
    const styleClass = BUTTON_STYLES[effectiveVariant]?.[effectivePalette] || BUTTON_STYLES['solid']['primary'];
  ...
  private getSizeClasses(shape: ButtonShape, size: ButtonSize): string {
    ...
    if (isPill) {
        switch (size) {
            case 'sm': return `px-3 py-1.5 text-xs ${rounded}`;
            case 'md': return `px-5 py-2.5 text-sm ${rounded}`;
            case 'lg': return `px-6 py-3 text-base ${rounded}`;
        }
    } else {
        switch (size) {
            case 'sm': return `p-1.5 ${rounded}`;
            case 'md': return `p-2.5 ${rounded}`;
            case 'lg': return `p-3.5 ${rounded}`;
        }
    }
    return '';
  }
